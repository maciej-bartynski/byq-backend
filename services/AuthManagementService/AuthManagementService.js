...
const AuthManagementService = {
    ...
    fetchManagementAccessToken() {
        if (process.env.MANAGEMENT_TEST_TOKEN) {
            setTimeout(() => {
                const token = process.env.MANAGEMENT_TEST_TOKEN;
            ...
            const options = {
                method: 'POST',
                url: process.env.MANAGEMENT_TOKEN_REQUEST_OPTIONS_URL,
                headers: { 'content-type': 'application/json' },
                body: process.env.MANAGEMENT_TOKEN_REQUEST_OPTIONS_BODY
            };
    ...
    fetchAuth0Users(req, res) {
        ...
        if (process.env.USE_FAKE_OTHER_USERS === 'true') {
            res.status(200).json({
                message: 'Users',
